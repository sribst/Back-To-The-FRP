...
let rf s t o = S.refine s (T.of_int t) o

let obs s name t =
  try S.observe s (T.of_int t)
  with Not_found ->
    Printf.printf "%s Not_found %d\n" name t ;
    raise Not_found
...
let bsc_d _ =
  ...
  let rf_l = [(0, 0); (10, 10); (20, 20); (30, 30)] in
  let time_l = [0; 10; 20; 30] in
  let exp_l = [0; 10; 20; 30] in
  let exp_interval_l = [[]; []; []; []] in
...
let _ =
  bsc_d () ; bsc_c () ; map_d () ; map_c () ; complete () ; map2 () ; fix ()
