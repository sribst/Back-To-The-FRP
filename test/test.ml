...
module To_test = struct
  ...
  let timer_continue refine_fct refine_list observe_fct time_list =
    (** retourne une liste de (booleen,t) disant si le timer
        entourant l'observation a était inférieur à 0.1s *)
    let aux (time,value) time' =
      refine_fct time value;
      observe_fct time'
    in
...
let rec test_list ol el ppv tv en =
  ...
  |h::t,h'::t' ->
    (ppv h h',
     `Slow,
     (fun () -> Alcotest.(check tv) en h h')
    )::(test_list t t' ppv tv en)
  |_ -> raise Not_same_size 
...
let run and_exit fl n ppv en tv rf rl o_f tl el  =
  try
    Alcotest.run ~and_exit:and_exit n
		 [n,fl ppv en tv rf rl o_f tl el ]
  with Not_same_size -> Printf.printf "expected list and observed_list are not of the same size" 
...
let run_timer_cont ?(and_exit = true) n = run and_exit (timer_continue_list)  (n ^ " timer continue") (string_of_float)
let run_timer_then ?(and_exit = true) n = run and_exit (timer_then_list)      (n ^ " timer then") (string_of_float)
